...
public class PaiementControlleur {
    ...
    @PostMapping("/processLastEvent")
    public CompletableFuture<ResponseEntity<String>> processLastEvent() {
        return paiementService.processLastEvent()
                .thenApply(v -> ResponseEntity.status(HttpStatus.OK).body("Last event processed successfully"))
                .exceptionally(e -> ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body("Error processing last event: " + e.getMessage()));
    }
}
